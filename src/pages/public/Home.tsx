...
export function Home() {
  ...
  const services = [
    { title: 'Seva Contributions', desc: 'Sponsor daily rituals, abhishekams, and archana services at the mandir.', icon: Flame, key: 'seva_enabled' },
    { title: 'Annadanam', desc: 'Contribute to daily free meals for devotees and the underprivileged.', icon: Heart, key: 'donations_enabled' },
    { title: 'Asset Offerings', desc: 'Donate gold, silver ornaments, or processional vehicles securely.', icon: Building2, key: 'assets_enabled' },
    { title: 'Events Seva', desc: 'Register to serve the community during Navratri and festivals.', icon: Users, key: 'events_enabled' }
  ].filter(s => isSectionVisible(s.key));

  return (
    <div className="flex flex-col bg-[#FDFBF7]">
      {/* Redesigned Premium Hero Section */}
      {isSectionVisible('hero_enabled') && (
        <section 
        ...
        >
          ...
          <div className="relative z-10 max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center">
            ...
            <div className="lg:col-span-6 flex flex-col items-center lg:items-start text-center lg:text-left space-y-6">
              ...
              {isSectionVisible('welcome_enabled') && (
              ...
              >
                {isSectionVisible('donations_enabled') && (
                ...
                {isSectionVisible('about_enabled') && (
                ...
                {isSectionVisible('events_enabled') && (
              ...
              {isSectionVisible('statistics_enabled') && (
                <motion.div 
                  initial={{ opacity: 0, y: 15 }}
                  animate={{ opacity: 1, y: 0 }}
                ...
                >
                  {(() => {
                    const stats = [
                      { value: `${publicStats?.yearsOfHeritage || 42} Years`, label: 'Heritage Legacy', key: 'stat_heritage_enabled' },
                      { value: `${publicStats && publicStats.registeredDevotees !== undefined ? publicStats.registeredDevotees.toLocaleString() : '0'}+`, label: 'Devotee Family', key: 'stat_devotees_enabled' },
                      { value: `₹${((publicStats && publicStats.totalAssetsValuation !== undefined ? publicStats.totalAssetsValuation : 0) / 10000000).toFixed(1)} Cr`, label: 'Assets Value', key: 'stat_assets_enabled' },
                      { value: `${publicStats && publicStats.activeCommitteeCount !== undefined ? publicStats.activeCommitteeCount : 0} Active`, label: 'Committee Members', key: 'stat_committee_enabled' }
                    ].filter(stat => isSectionVisible(stat.key));

                    if (stats.length === 0) return null;

                    const gridColsClass = stats.length === 1 
      ...
      {/* Quick Info Cards / Temple Highlights */}
      {isSectionVisible('highlights_enabled') && (
      ...
      {(() => {
        const showDevotees = isSectionVisible('stat_band_devotees_enabled');
        const showFunds = isSectionVisible('stat_band_funds_enabled');
        const showLedger = isSectionVisible('stat_band_ledger_enabled');
        const visibleCount = [showDevotees, showFunds, showLedger].filter(Boolean).length;

        if (!isSectionVisible('statistics_enabled') || visibleCount === 0) return null;
        ...
        const divideClass = visibleCount > 1 ? 'divide-y sm:divide-y-0 sm:divide-x divide-[#EEDCC1]/50' : '';

        return (
          <section className="py-12 bg-white mt-12 border-y border-[#EEDCC1]/50">
            <div className={`mx-auto px-4 grid gap-8 sm:gap-4 text-center font-sans text-xs ${gridCols} ${divideClass}`}>
              {showDevotees && (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
              ...
              <div className="space-y-6">
                ...
                <div className="flex flex-col sm:flex-row gap-4 pt-2">
                  {isSectionVisible('temple_history_enabled') && (
                  ...
                  {isSectionVisible('committee_enabled') && (
